...
@Entity(name = "Lymov0f")
@NamedQueries({
	@NamedQuery(name = "AsLymov0f.findAll", query = "SELECT c FROM Lymov0f c"),
	@NamedQuery(name = "AsLymov0f.findToElab", query = "SELECT c FROM Lymov0f c WHERE c.lysta = :lysta order by c.id.lydat, c.id.lycau, c.id.lynuz, c.id.lynum, c.id.lyprg"),
	@NamedQuery(name = "AsLymov0f.updateElab", query = "UPDATE Lymov0f c SET c.lysta = :lystaElab WHERE c.lysta = :lystaNonElab and c.id.lydat = :lydat and c.id.lycau = :lycau and c.id.lynuz = :lynuz and c.id.lynum = :lynum")
})
public class AsLymov0f implements Serializable{
	...
	public String toString(){
		...
		stringBuilder.append("lysta=" + StringUtils.trim(lysta));
		stringBuilder.append("]");
